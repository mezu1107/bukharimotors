...
import { AlertTriangle, Calendar, Car, CheckCircle2, Clock, FileDown, FileText, Loader2, Receipt, RefreshCw, TrendingUp, Users } from "lucide-react";
...
type VehicleRow = { id: string; make: string; model: string; registration_no: string; status: string };
...
function Dashboard() {
  ...
  const load = async () => {
    ...
    const [bookings, clients, vehicles, payments] = await Promise.all([
      supabase
        .from("bookings")
        .select("id, booking_no, client_id, vehicle_id, status, pickup_at, dropoff_at, total_amount, advance_amount, balance_amount, created_at")
        .order("created_at", { ascending: false })
        .limit(1000),
      supabase.from("clients").select("id, full_name, phone").order("created_at", { ascending: false }).limit(1000),
      supabase.from("vehicles").select("id, make, model, registration_no, status").order("created_at", { ascending: false }).limit(1000),
      supabase.from("payments").select("amount, paid_at").order("paid_at", { ascending: false }).limit(1000),
    ]);
  ...
  const summary = useMemo(() => {
    ...
    const sales30 = data.bookings.filter((b) => new Date(b.created_at).getTime() >= last30).reduce((sum, b) => sum + Number(b.total_amount ?? 0), 0);
    return { active, completed, bookingSales, advances, balances, received, availableVehicles, sales30 };
  }, [data]);
  ...
  const trend = useMemo(() => {
    ...
    return Object.entries(buckets).map(([day, amount]) => ({ day, amount, pct: Math.max(6, (amount / max) * 100) }));
  }, [data.bookings]);
  ...
  if (authLoading || loading) {
    return <div className="flex min-h-[55vh] items-center justify-center"><Loader2 className="size-7 animate-spin text-primary" /></div>;
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Real database totals for bookings, sales, clients, vehicles, balances and downloads.</p>
        </div>
        <Button variant="outline" onClick={load}><RefreshCw className="mr-2 size-4" /> Refresh</Button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Metric label="Received" value={fmtMoney(summary.received || summary.advances)} icon={Receipt} />
        ...
        <Metric label="Available Cars" value={`${summary.availableVehicles}/${data.vehicles.length}`} icon={Car} />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="border bg-card p-5 shadow-sm lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-muted-foreground">Based on booking totals from the database</div>
            </div>
          ...
          <div className="flex h-56 items-end gap-3 rounded-lg bg-secondary/50 p-4">
            {trend.map((item) => (
              <div key={item.day} className="flex h-full flex-1 flex-col justify-end gap-2 text-center">
                <div className="flex flex-1 items-end">
                  <div className="w-full rounded-t-md bg-primary" style={{ height: `${item.pct}%` }} />
                </div>
        ...
        <Card className="border bg-card p-5 shadow-sm">
          ...
          <div className="space-y-2 text-sm">
            <Link to="/bookings/new" className="flex items-center gap-3 rounded-md p-3 transition hover:bg-accent"><FileText className="size-4 text-primary" /> New Rental Agreement</Link>
            <Link to="/bookings" className="flex items-center gap-3 rounded-md p-3 transition hover:bg-accent"><FileDown className="size-4 text-primary" /> Search & Download PDFs</Link>
            <Link to="/clients" className="flex items-center gap-3 rounded-md p-3 transition hover:bg-accent"><Users className="size-4 text-primary" /> Clients</Link>
            <Link to="/vehicles" className="flex items-center gap-3 rounded-md p-3 transition hover:bg-accent"><Car className="size-4 text-primary" /> Fleet</Link>
          </div>
        </Card>
      </div>
      ...
      <Card className="border bg-card p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-muted-foreground">Latest records pulled live from database</div>
          </div>
        ...
        <div className="space-y-3">
            ...
            return (
              <div key={b.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3">
                <div className="min-w-0">
                  ...
                  <div className="mt-1 truncate text-sm font-semibold">{client?.full_name ?? "Unknown client"} <span className="font-normal text-muted-foreground">• {vehicle ? `${vehicle.make} ${vehicle.model} (${vehicle.registration_no})` : "Unknown vehicle"}</span></div>
                  <div className="text-xs text-muted-foreground">{fmtDateTime(b.pickup_at)} → {fmtDateTime(b.dropoff_at)}</div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <Button variant="outline" size="sm" disabled={downloading === b.id} onClick={() => download(b.id)}>
                    {downloading === b.id ? <Loader2 className="mr-1 size-4 animate-spin" /> : <FileDown className="mr-1 size-4" />}
                    PDF
                  </Button>
                </div>
              </div>
          ...
          {recentBookings.length === 0 && <div className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">No booking records found in the database.</div>}
        </div>
      </Card>
    </div>
  );
